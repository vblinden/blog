...
In the past it was quite easy to see which websites used the popular Bootstrap framework for styling. They all kinda looked the same, especially if they didn't spend much time customizing the framework. The site usually had the distinct blue primary color navbar and buttons, and it used the jumbotron and grid layout components in a recognizable way.

Too many times I have seen companies wanting to differentiate themselves from that particular "Bootstrap" look. Not by customizing the Bootstrap framework (or any other framework for that matter), but by writing their own CSS framework from scratch. Which is almost always a very bad idea.
...
Instead of building your own, you should use an existing CSS framework. For example, just use Tailwind CSS. They provide sane defaults, they provide updates, they provide documentation, they provide fixes, and new hires can Google questions, ask AI, or draw on previous experience because, chances are, they've used it (and loved it) at previous companies. There is a reason Tailwind CSS took the CSS world by storm since its conception and is the most used CSS framework since 2020 according to <a href="https://stateofcss.com" target="_blank">stateofcss.com</a>.

It is very easy to customize Tailwind CSS with, for example, <a href="https://tailwindcss.com/docs/theme" target="_blank">themes</a>. Want different colors for your theme? Different radius? Different text sizes? You can just use <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_cascading_variables/Using_CSS_custom_properties" target="_blank">CSS custom properties</a> and import the theme in your company CSS and be done with it.
